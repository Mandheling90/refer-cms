...
import { commonCodeApi } from '@/lib/api/code';
import type { CommonCode } from '@/types/code';
import { toast } from 'sonner';
import { DEFAULT_PAGE_SIZE } from '@/lib/constants';

const columns: ColumnDef<CommonCode, unknown>[] = [
  { accessorKey: 'CODE_GROUP', header: '코드 그룹', size: 150 },
  { accessorKey: 'CODE_GROUP_NAME', header: '그룹 이름', size: 150 },
...
export default function CommonCodePage() {
  ...
  const retrieveList = useCallback(async (page = currentPage, size = pageSize) => {
    ...
    try {
      ...
      const res = await commonCodeApi.list(params);
      setData(res.list || []);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await commonCodeApi.save(formData);
  ...
  const handleDelete = async () => {
    try {
      const res = await commonCodeApi.remove(selectedRows);
      if (res.ServiceResult.IS_SUCCESS) {
  ...
  return (
    <>
      <ListPageTemplate
        ...
        formSection={
          <CrudForm title="공통 코드 추가/수정" onAdd={() => setFormData({})} onSave={handleSave}>
